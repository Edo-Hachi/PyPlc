...
## 🔄 **次期開発計画: ダイアログシステムリファクタリング（準備中）**

### **背景と課題**
- **main.py肥大化**: 705行（ダイアログ関連65行含む）
- **責任分散**: UIロジック、ダイアログ処理、アプリ制御の混在
- **拡張困難**: 新ダイアログ追加時のmain.py修正必要

### **提案アーキテクチャ**
```
dialogs/                    # 新規独立ディレクトリ
├── __init__.py            # エクスポート管理
├── device_id_dialog.py    # 移動後のダイアログクラス
└── dialog_manager.py      # 統合管理クラス（新規）
```

### **期待効果**
- **main.py削減**: 705行 → 640行（9%削減）
- **責任分離**: アプリ制御 vs ダイアログ管理の明確化
- **拡張性**: 将来のダイアログ追加容易化

### **実装プラン**
- **計5フェーズ**: 1時間50分の段階実装
- **詳細仕様**: `_Claude_Plan.md`に記載済み
- **実行待ち**: ユーザー承認後開始予定

---

*最終更新: 2025-08-06*  
*更新内容: Phase 5完了（デバイスID編集システム）、ダイアログリファクタリング計画策定*  
*次回更新: ダイアログシステムリファクタリング実行時*